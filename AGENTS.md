...
- **Google login dentro del APK (WebView)**: integrado con `@capgo/capacitor-social-login` (Credential Manager nativo). En `AuthDialog.tsx` el botón de Google detecta Capacitor (`isNative()`) y usa `SocialLogin.initialize({ google: { webClientId } })` + `SocialLogin.login()` → idToken → `signInWithCredential` en el JS SDK. En la web sigue usando `signInWithPopup`. Requisito: OAuth Client de Android en Google Cloud Console (package `com.varius.app` + SHA-1 del keystore) y `webClientId` (debe ser un OAuth Client tipo Web).
